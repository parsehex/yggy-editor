...
export function initDialogueTabEvents() {
	...
	// update dialogue
	_editorDelegate(
		`${tab} textarea.dialogue`,
		'input',
		(e, t: HTMLTextAreaElement) => {
	...
	_editorDelegate(
		`${tab} div.choices button.create-dialogue`,
		'click',
		(e, t) => {
			const d = getData('dialogue', gameState.currentDialogueID);
	...
	// go to dialogue from choice
	_editorDelegate(
		`${tab} div.choices button.go-to-dialogue`,
		'click',
		(e, t) => {
			const choiceId = +t.closest('div.choice').dataset.id;
			const choice = getData('choices', choiceId);
			if (gameState.currentDialogueID === choice.targetDialogueID) return;
			gameState.currentDialogueID = choice.targetDialogueID;
			draw();
		}
	);
	...
	// change character
	_editorDelegate(
		`${tab} div.characters .character select.name`,
		'click',
		(e, t: HTMLSelectElement) => {
	...
	_editorDelegate(
		`${tab} div.characters .character select.frame`,
		'click',
		(e, t: HTMLSelectElement) => {
			const isFirst = t.closest('div.character').matches(':first-of-type');
			const d = getData('dialogue', gameState.currentDialogueID);
	...
	_editorDelegate(
		`${tab} div.characters .character input.owner`,
		'change',
		(e, t: HTMLSelectElement) => {
			const d = getData('dialogue', gameState.currentDialogueID);
